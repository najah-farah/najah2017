<?php
	...
	if(isset($_POST["register"]))
	{
		...
		if(!isset($error)){
		if(mysqli_query($connection, "INSERT INTO `photographer`(`firstName`, `lastName`, `mobileNumber`, `password`, `email`, `profile`, `about`, `status`) VALUES('" . ucwords(strtolower(trim($_POST["firstName"]))) . "', '" . ucwords(strtolower(trim($_POST["lastName"]))) . "', '" . trim($_POST["mobileNumber"]) . "', '" . trim($_POST["password"]) . "', '" . trim($_POST["email"]) . "', '$fileName1', '" . trim($_POST["about"]) . "', 'OFF')"))
		{
